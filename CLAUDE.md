...
- Perform tasks step by step. Do not undertake anything or add variables, functions, imports, files, or other artifacts if they are not used immediately or are not required for the current task.
- Instructions for every step:
  1. Think and understand the task in the context before start working
  2. Execute the current task as a single, self-contained step
  3. Only create code or artifacts that are directly used in this step's output
  4. Never declare variables, functions, imports, types, files, or other elements that will not be used immediately
  5. If a resource will be needed later, postpone its creation until the step where it will be used
  6. Keep changes minimal — only what is necessary to complete the current step
  7. When responding, show the step taken and its direct result; avoid adding auxiliary scaffolding or placeholders
